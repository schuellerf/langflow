...
| model_kwargs | Dict | Input parameter. Additional keyword arguments to pass to the model. Note: `stream_options` is disabled by default for compatibility with vLLM/Gemini. |
| json_mode | Boolean | Input parameter. If True, it will output JSON regardless of passing a schema. |

## Compatibility Notes

The vLLM component automatically disables `stream_options` by default to ensure compatibility with different vLLM deployments and providers like Google Gemini. This prevents errors such as "Unknown name 'continuous_usage_stats'" that can occur when using OpenAI-compatible APIs that don't support all streaming options.

If you need to enable `stream_options` for your specific vLLM deployment, you can override this behavior using the `model_kwargs` parameter:
